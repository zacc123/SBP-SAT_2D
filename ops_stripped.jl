...
## Adding 1D version
function get_operators(p, Nx, μ; xc = (-1, 1))
...
# Compatible SBP
  ...
  # penalty matrices:
  Z_1 = β * (d/h1) * μ *  (Ix) # TODO: are these correct?
  Z_2 = β * (d/h1) * μ *  (Ix)
  ...
  # Dirichlet on faces 1 and 2:
  SAT_1 = Hinv * (T_1 .- Z_1)' * e1 * e1'   #TODO: fix these with A&D paper
  SAT_2 = Hinv * (T_2 .- Z_2)' * e2 * e2'   #TODO: fix these with A&D paper
  ...
  D2 = μ * (D2x) 
  A = D2 + SAT_1 + SAT_2 # Creation of LHS matrix

  B = (B1, B2)  # These will multiply the boundary data when forming the RHS.

  H̃ = Hx            # Need this for convergence tests.

  return (A, B, H̃, T, e)
end

function bdry_vec_strip!(g, B, x, z, slip_data, remote_data,  Lx, Lz)
...
function bdry_vec_strip!(g, B, x, slip_data,   Lx)
