...
function converge_2D_TSIT(exact; dt=1e-4, dy=0.1, dz=0.1, tc = (0, 5), yc=(-1, -1), zc = (-1, 1))
    ...
    for i in eachindex(dys)
        ...
        print("\nTiming for Error Measurement:\n")
        @time error = get_error_2d(res, Y_GRID, Z_GRID, sol.t[end], exact)
        space_errors[i] = error * sqrt(dr*ds)
        print("\nL2 Error: $(error * sqrt(dr*ds))\n")
