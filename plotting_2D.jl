...
function plot_2d_tsit!(Y_GRID, Z_GRID, T_GRID, result, exact; file_dir = "./result_imgs/")
...
function plot_2d!(Y_GRID, Z_GRID, T_GRID, result, exact; file_dir = "./result_imgs/")
    ...
    # Look at z = Zgrid[5], y = Ygrid[5]
    plot(T_GRID, result[N, :], label="numerical")
    plot!(T_GRID, [exact(t, Y_GRID[end], Z_GRID[end]) for t in T_GRID],label="exact")
    png("./result_imgs/T at Y=0, T=0")
